...
import { VoiceTranscriptCard } from "@/src/components/organisms/voicerx/VoiceTranscriptCard";
 ...
 */
function getCompletenessForOutput(output) {
 ...
 */
function getSourcesForOutput(output, documents) {
  if (!output) return null;
  ...
  switch (output.kind) {
    ...
    case "patient_narrative":{
        const d = output.data;
        const entries = [{ label: "EMR", description: "Patient's clinical data" }];
    ...
    case "pomr_problem_card":{
        const d = output.data;
        const entries = [{ label: "History", description: `${d.problem} clinical data` }];
...
function PortalTooltip({
  ...
  return createPortal(
    <div
      ...
      <div
        className={cn("rounded-[8px] bg-white overflow-hidden", styles.tooltipCard)}>
        
        {children}
      </div>
      {/* Downward arrow — paddingLeft is dynamic (pos.arrowLeft - 5), kept inline */}
      {pos &&
  ...
  return (
    <>
      ...
      {isOpen && typeof document !== "undefined" && createPortal(
        <div
          ref={dropdownRef}
          className={cn("fixed z-[9999] w-[280px] rounded-[8px] bg-white overflow-hidden", styles.sourceDropdown)}
          ...
          <div
            className={cn("flex items-center gap-[5px] px-[12px] py-[7px]", styles.sourceDropdownHeader)}>
...
function renderAssistantMarkdown(text, onPillTap) {
...
export function ChatBubble({
  ...
  // animation finished.
  const [shimmerActive, setShimmerActive] = useState(() => !isUser && !!message.text && !!message.shimmerReveal);
  const [streamActive, setStreamActive] = useState(() => !isUser && !!message.text && !message.shimmerReveal);
  ...
  if (isUser) {
    return (
      <div className="group/msg flex w-full flex-col">
        ...
        <div className={cn("w-full rounded-[12px] border border-tp-blue-200 bg-white px-3 py-2.5", styles.editContainer)}>
            <textarea
            ref={editRef}
            value={editText}
            ...
            <div className="mt-[8px] flex items-center justify-end">
              <div className="flex gap-[6px]">
                ...
                <button type="button" onClick={handleSaveEdit} disabled={!editText.trim() || editText.trim() === userDisplayText} className={cn("rounded-[6px] px-[10px] py-[4px] text-[12px] font-medium text-white transition-colors disabled:opacity-40", styles.editSendBtn)}>Send</button>
              </div>
            </div>
          </div>) :
      ...
      <div className="flex w-full flex-col items-start">
            ...
            (() => {
              ...
              return message.text.split(/(\n)/).map((line, li) => {
                ...
                return line.split(/(\*\*[^*]+\*\*|\s+)/).filter(Boolean).map((token, ti) => {
                  if (/^\s+$/.test(token)) {
                    return <span key={`${li}-${ti}`} className="da-shimmer-word" style={{ "--da-delay": `${wordIdx * 70}ms` }}>{token}</span>;
                  }
                  ...
                  wordIdx++;
                  return isBold ?
                  <strong key={`${li}-${ti}`} className="da-shimmer-word font-semibold text-tp-slate-900" style={{ "--da-delay": `${delay}ms` }}>{text}</strong> :

                  <span key={`${li}-${ti}`} className="da-shimmer-word" style={{ "--da-delay": `${delay}ms` }}>{text}</span>;
          ...
          {(() => {
            ...
            return (
              <>
                ...
                <>
                    <div
                    className={cn("h-[12px] w-[1px] flex-shrink-0", styles.dividerLine)} />
        ...
        <div className="ml-[30px] mt-[8px] w-[calc(100%-30px)]">
            ...
            <button
              ...
              className={cn("flex-shrink-0 rounded-full px-[10px] py-[5px] text-[12px] font-medium transition-all duration-150 active:scale-[0.97]", styles.suggestionPill)}>
              
                  <span className={styles.suggestionPillText}>{s.label}</span>
                </button>
            )}
            </div>
          </div>
