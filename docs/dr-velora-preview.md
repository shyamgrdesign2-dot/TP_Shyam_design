...
Use the header search or Dr.Velora launcher. The panel expands to a modal or docks on the right. The original Doctor greeting and compact suggestion chips are preserved; the welcome composer sits below the chips with sync immediately below it; the welcome input is slightly taller and receives focus whenever the panel opens; the privacy marker stays at the bottom. Rx pad VoiceRx opens the existing consultation-mode chooser. A single mounted panel preserves its thread and native voice state while changing presentation.
...
Cards reuse the existing CardShell and demonstrate narrative citations, measurement/reference tables, prescription dose/schedule/duration, and a clickable trend. A citation opens the evidence drawer at its selected source. The evidence footer opens all sources. The Tesseract Drawer has one fixed header followed directly by numbered collapsible records. Each sample record displays a complete PDF (generated locally with pdf-lib and rendered by PDF.js), with a document title and date. Evidence Text appears above the PDF, with supplied evidence ranges highlighted soft yellow. The PDF has no fixed viewer header or footer; pages scroll within a vertically resizable region. The gray viewport has a normal vertical scrollbar and side padding only. A 12px top gap inside the page content scrolls away with the first page; there is no fixed top/bottom inset. A bottom-floating pill shows the current/total page count, previous/next-page buttons and a download action. Page selection follows scrolling and resizing; downloads use the same locally rendered PDF. One Record details footer starts collapsed and expands to show document metadata. Cards use a white-to-violet gradient header with a violet record number, a neutral PDF-viewer background and a white Record details footer. Both answer cards and the drawer use the exact Tesseract rounded/bulk/school-learning/note-2.svg evidence icon; the drawer icon stays neutral, with a blue record-count badge beside Evidence. The header carries the drawer’s only sync timestamp, followed by an info icon; the whole sync control opens Record Sync Details on hover or click. Accordion arrows sit on the right. Clinician rows, format badges, visit ordinals and redundant tags are omitted. It has no fixed footer. Escape closes the evidence drawer without dismissing the parent panel.
...
Tesseract is built locally from the supplied Storybook checkout and installed from `vendor/dhspl-tatvacare-tesseract-ui-1.1.0.tgz`. Both `package.json` and `package-lock.json` point to this committed archive, so Vercel does not fetch Tesseract from GitHub Packages. No `GITHUB_PACKAGES_TOKEN`, `NPM_TOKEN` or private registry `.npmrc` is required. The remaining dependencies install from public npm. The existing Tesseract CDN icons remain in use.

See `vendor/README.md` for source provenance and update instructions. Redeploy the latest main commit, rather than retrying the older token-based deployment.


## Sync and evidence UX update

The patient search ends with a subtle divider and the bulk microphone icon. Last synced sits at the right end of the chips row below the input. Lab records and Prescriptions retain their bulk icons under the neutral Available records heading, separated from the freshness note by a divider. Expanded conversations have up to 64px horizontal padding; the side dock uses 20px. It opens question dictation in the copilot composer; recognized text is kept for review before sending. Browsers without speech recognition offer a typed-question fallback. The same patient sync tooltip is available by hover, keyboard focus or click from the header, welcome sync, message sync and evidence drawer. It uses the patient name as its heading with age and gender beneath, followed by an unboxed sync timestamp and available lab-result and visit counts. Response and evidence tooltips retain the answer's original sync timestamp.

User-facing sample badges and repeated fictional/no-live-sync notes are omitted. The underlying fixtures remain explicitly synthetic in code; this change does not connect additional clinical data. Evidence places exact source text above the PDF, highlighting only supplied character ranges in soft yellow. PDF content is unmarked. Record Details starts collapsed below the document.

Welcome sync appears below the 84px input, which receives focus on opening or changing panel presentation. Tooltip freshness guidance reads “Records added after the sync may not be included.” with an amber warning icon and text.
